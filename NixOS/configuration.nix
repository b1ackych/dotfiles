...
#keyMap = "us";
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.writers.writeBash "external_monitor_script" ''
          ...
          if (xrandr | grep "$EXT disconnected"); then
              xrandr --output $EXT --off --output $IN --auto --dpi 120 && echo "Internal monitor is set"
          else
              xrandr --output $IN --off --output $EXT --auto --mode 2560x1440 --rate 75 --dpi 100 && echo "External monitor set to HDMI-2"
          fi
      ''}";
  ...
  '';
  #KERNEL=="card0", SUBSYSTEM=="drm", ACTION=="change", ENV{HOTPLUG}=="1", RUN+="${pkgs.systemd}/bin/systemctl --no-block start monitor_script.service"
